...
texclean()
{
    ...
    case "$1" in 
        ...
        -h | -?)
            echo "Syntax: texclean [-{r|R}]"
            echo "  removes tex detritus from the current directory"
            echo "  or recursively with -r or -R"
        ;;
...
undeux()
{
    pushd ~/python.programming/undeux >/dev/null
    python undeux.single.file.py $@
    popd >/dev/null
}

hosts()
{
    grep ^[H\h]ost ~/.ssh/config | awk '{print $2}' | sort
}
...
slurmwriter()
{
    pushd ~/python.programming/slurm >/dev/null
    clear
...
{
    if [ -z $1 ]; then
        echo 'Usage: findtext {textshred} ["filespec"]'
        echo " "
        echo " This function will locate text shreds in text files,"
        echo " and it allows you to look only in an optional wildcard"
        echo " match of file names to inspect. The search begins in "
        echo ' $PWD, which is currently '$PWD
        return
    fi
    echo "find . -name \"$2\" -exec grep -HEn {} \; 2>/dev/null"
    command find . -name \"$2\" -exec grep -HEn {} \; 2>/dev/null
}

function findtoken
{
    if [ -z $1 ]; then
        echo "Usage: findtoken {textshred} [filespec]"
        echo " "
        echo " This function will locate tokens in text files, and"
        echo " and it is sensitve to the type of file to define"
        echo " what a token looks like."
        return
    fi 
    tok="\\<$1\\>"
    findtext $tok $2
...
panagram()
{
    cd ~/python.programming/anagrammatic
    python3 panagrammar.py --dictionary mydict $@ 
    back
}
...
{
    cd ~/python.programming/anagrammatic
    python3 anagrammar.py --dictionary mydict $@ 
    back
}
...
if [ ! -z `which git 2>/dev/null` ]; then
    ...
    source ~/bin/git.bash 2>&1 >/dev/null
else
